...
public final class IStream {
     ...
     * {@link SofabError#INVALID_MSG} it is exactly how a stream decoder resynchronises
     * onto the next message. {@code acc} is not cleared: only its first
     * {@code accLen} bytes are ever read, and that is zeroed here. Neither are
     * {@code scratchPos} / {@code tailBits}, which every path writes before it
     * reads them.
     */
    public void reset() {
        varintValue = 0;
